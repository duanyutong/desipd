...
grid_size = 1 # (mm) grid goes from -grid_size to grid_size. The step size is determined by this and the num_grid, whch is the number of points between -grid_size and grid_size.
etime = 10. #sys.argv[1]

#set a folder for the new fits files to be saved in so they are safe from clobber
...
def create_fits(data,num,type,dir):
    ...
    hdu.writeto(dir+'FPC_'+str(num)+'.fits')
...
def move_and_fpc_measure(command,targets):
    ...
    for i,target in enumerate(targets):
        ...
        #move_dir = final_local_dir+'/'+str(i)+'/'
        #if not os.path.exists(move_dir):
        #    os.makedirs(move_dir)
        #print("Images being saved in %s" % move_dir)
        image_num = iter(np.arange(3*n_frames))
        for x in range(n_frames):
            # fpc_expose(next(image_num),'bias',move_dir)
            #time.sleep(1)
            #fpc_expose(next(image_num),'dark',move_dir)
            #time.sleep(1)
            fpc_expose(next(image_num),'object',move_dir)
            time.sleep(1)
