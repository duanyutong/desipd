...
def reduce_dataset(path_dataset,
    ...
    logging.basicConfig(filename = os.path.join(
                                        path_dataset, 'data_reduction.log'),
                        level = logging.INFO,
                        format = '%(asctime)s: %(message)s',
                        datefmt= '%Y/%m/%d %H:%M:%S')
    print('Log file is saved to :', path_dataset)
...
#    logging.warning('And this, too')
    ...
    if path_flat is None:
        ...
        logger.info('Flat provided, using flat at {}'.format(path_flat))
        ...
        if 'master.fits' in os.listdir(path_dataset) and not clobber:
