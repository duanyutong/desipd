...
data_dir = '/home/protodesi/Desktop/data/'
exptime = 100 #ms
rest = 5 #sec 
DARK = False
num = 1 #'cont'= continuous
name = 'metrology' #filename identified, will include time
shutt = False #True shutter closes, False shutter stays open 
window_mode=False
################
...
if num=='lin_test':
	...
	for i in EXP:
		...
		filename = (name+"%04d"+'_'+datetime.now().strftime("%H:%M:%S.%f")+'.fits') % (i,) 
	...
	while N>0:
		...
		image, start = take_image
		#print(start)
		filename = (name+"%02d"+'_'+datetime.now().strftime("%H:%M:%S.%f")+'.fits') % (N,)
	...
	try:
		while True:
			...
			take_image = camera.start_exposure(shutter=shutt)
			image, start = take_image
			filename = name+datetime.now().strftime("%H:%M:%S.%f")+'.fits'
			camera.write_fits(image,data_dir+'/'+filename,exptime,start,get_moon(start))
			time.sleep(rest)
